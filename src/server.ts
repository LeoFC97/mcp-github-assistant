  ...
  async ({ owner, repo, state, labels }) => {
    try {
      ...
      const list = issues.map(i =>
        `• #${i.number} — ${i.title}\n  Author: ${i.user?.login} | Labels: ${i.labels.map((l: any) => l.name).join(", ") || "none"} | Updated: ${i.updated_at}`
      ).join("\n\n");
...
server.prompt(
  ...
  async ({ owner, repo, pr_number }) => ({
    messages: [
      {
        role: "user",
        content: { type: "text", text: `Please review PR #${pr_number} in ${owner}/${repo}.` }
      },
      {
        role: "assistant",
        content: {
          type: "text",
          text:
            "I will review this PR as a senior developer, evaluating:\n" +
...
server.prompt(
  ...
  async ({ owner, repo }) => ({
    messages: [
      {
        role: "user",
        content: { type: "text", text: `Please triage the open issues in ${owner}/${repo}.` }
      },
      {
        role: "assistant",
        content: {
          type: "text",
          text:
            "I will triage the open issues, classifying each by:\n" +
